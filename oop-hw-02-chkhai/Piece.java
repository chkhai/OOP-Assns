...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		//Arrays.sort(body);
		Arrays.sort(body, Piece::compare_points);
		...
		width = x_max - x_min + 1;
		skirt = new int[width];
		for(int i = 0; i < skirt.length; i++) skirt[i] = Integer.MAX_VALUE;
		for(TPoint p : body) {
			int idx = p.x - x_min;
			if(p.y < skirt[idx]) skirt[idx] = p.y;
		}
		//done
	}

	private static int compare_points(TPoint p1, TPoint p2) {
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] new_arr = new TPoint[body.length];
		for(int i = 0; i < body.length; i++) {
			int n_h =  getHeight() - body[i].y - 1;
			int n_w =  body[i].x;
			TPoint new_point = new TPoint(n_h, n_w);
			new_arr[i] = new_point;
		}
//		System.out.println(Arrays.toString(new_arr));
		// YOUR CODE HERE
		return new Piece(new_arr); // YOUR CODE HERE
		//done
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		Arrays.sort(other.getBody(), Piece::compare_points);
		// YOUR CODE HERE
		return Arrays.equals(other.getBody(), this.getBody());
		//return true;
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		while(!root.equals(curr)) {
			prev = curr;
			Piece dummy = curr.computeNextRotation();
			curr.next = dummy;
			curr = dummy;
